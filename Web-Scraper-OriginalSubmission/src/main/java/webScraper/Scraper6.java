 ...
 */
public class Scraper6 extends Thread {
    //Specifies the interval between HTTP requests to the server in seconds.
    private int crawlDelay = 6;
    ...
    public void run(){
        ...
        while(runThread){
            System.out.println("Scraper 6 thread is scraping data.");
            
          Document document6 = null;
            try {
                document6 = Jsoup.connect("https://www.chronext.co.uk/buy?q=Cartier").get();
        ...
        for (Element row : document6.select("div.products-list-watches.product-list-thumb a")) {
